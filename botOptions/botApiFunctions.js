...
import {catQ, dollarQ} from "../constants/botButtonsQuery.js";
import {
    botOptions,
    currencyOptionsEUR,
    currencyOptionsUSD,
...
dotenv.config();
export async function getCommands(chatId) {
...
export async function getWeather(chatId, city) {
    try {
...
function transpCelcius(deg) {
    return ((deg - 32) * 5 / 9).toFixed(1);
}
export function validWordInput(text){
    return /^[a-zA-Z]+$/.test(text)
}
export function validImageTitleInput(text){
    return /^[a-zA-Z0-9]+$/.test(text)
}
export async function getImage(chatId, animal) {
    if (animal === catQ) {
        try {
...
export async function getCurrency(chatId, currency, stayNotified) {
...
export async function getAdvice(chatId) {
    try {
...
export async function getJoke(chatId) {
    try {
        const config = {
            headers: {
...
export async function findImage(chatId, name) {
    try {
        const config = {
            headers: {
...
export async function getNews(chatId, text) {
    try {
