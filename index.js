...
import {
    ...
    editTaskTitle,
    getUserTasks
} from "./repo/taskController.js";
import {botOptions} from "./botOptions/buttonOptions.js";
import {
    adviceC,
    catC,
    dogC,
    ...
    startC,
    weatherC
} from "./constants/botCommands.js";
import {
    addCityI,
    changeTaskPropI, changeTaskTimeI,
    changeTaskTitleI, deleteCityI,
    deleteTaskI,
    editTaskI, findCityI,
    hoursI,
    imageI,
    newsI,
    titleI
} from "./constants/userInputCommands.js";
import {
    addCityQ,
    adviceQ, catQ,
    createTaskQ, deleteCityQ,
    deleteTaskQ, dogQ, dollarNoQ, dollarQ, dollarYesQ,
    editTaskQ, euroNoQ, euroQ, euroYesQ, findCityQ, helpQ,
    jokeQ,
    newsQ,
    searchQ,
    tasksQ, weatherQ
...
import {
    findImage,
    getAdvice, getCommands,
    getCurrency,
    getImage,
    getJoke,
    getNews,
    getWeather, validImageTitleInput, validWordInput
} from "./botOptions/botApiFunctions.js";
import {
    currencyNotification,
    getTasks,
    getUserWeather, subscribeCurrency,
    taskNotification,
    weatherNotification
} from "./botOptions/userNotifications.js";
...
const start = async () => {
    ...
    let taskNotify = null;
    let weatherNotify = null;
    let currencyNotify = null;
    ...
    bot.on('message', async msg => {
            ...
            if (text === startC &&!command) {
                ...
                if (isFinite(text) && Number(text) > 0 && Number(text) <= weatherNotifications.length && Number(text) % 1 === 0) {
                    await deleteWeatherNotify(username, weatherNotifications[text - 1]);
                ...
                if (isFinite(text) && text >= 1&&text <= 240) {
                    let taskId = await createTask(title, Date.now() + text * 3600000);
                ...
                if (isFinite(text) && Number(text) <= tasks.length && Number(text) > 0 && Number(text) % 1 === 0) {
                ...
                if (isFinite(text) && Number(text) < 4 && Number(text) > 0 && Number(text) % 1 === 0) {
                    if (text === '1') {
